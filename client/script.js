...
function changeButton(e){
    ...
    console.log(sayHelloButton, "function runs")
}
sayHelloButton.addEventListener("mouseenter", changeButton)
// PROBLEM 3
/*
...
const ohMy = () => {
    axios.get('http://localhost:3000/animals').then((res)=>{
        for (let i = 0; i<res.data.length; i++){
            let pTag=document.createElement("p").textContent=res.data[i]
...
const repeatMyParam = () => {
    axios.get(`http://localhost:3000/repeat/${"helloooo again"}`).then((res)=>{
...
// CODE HERE
function queryTime(){
    axios.get(`http://localhost:3000/query-test?age=30`).then(res=>{
        console.log(res)
    })
    ...
    axios.get(`http://localhost:3000/query-test?age=30&tall=true`).then(res=>{
        console.log(res)
    })
}
document.getElementById('query-button').addEventListener("click", queryTime)
...
function createFood(e) {
    e.preventDefault()
    let foodInput=document.getElementById("enter-food")
    ...
    let body={
        ...
        axios.post(`http://localhost:3000/food`, body).then((res)=>{
            console.log(res.data)
            for (let i = 0; i<res.data.length; i++){
                document.querySelector("section").append(document.createElement("h1").textContent=res.data[i])
        }})
}
